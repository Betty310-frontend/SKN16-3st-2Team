...
class ChatbotController:
    ...
    def send_topic_query(self, topic: str) -> Tuple:
        ...
        """
        if not self.app_state["user_session"]["auth"]:
            return self.app_state["chat_history"], "로그인 후 이용해 주세요.", "", self._sources_md()
        ...
        return self.app_state["chat_history"], status, "", self._sources_md()
    ...
    def _add_sources(self, source_documents):
        """챗봇 응답에서 받은 근거 자료들을 전역 evidence_library에 추가합니다."""
        for doc in source_documents:
            metadata = doc.metadata
            url = f"#source_{hash(str(metadata))}"
            title = metadata.get("source", "문서")
            
            if url not in self.app_state.get("source_bucket", {}):
                self.app_state.setdefault("source_bucket", {})[url] = {
                    "title": title, 
                    "first_seen": self._now_iso()
                }

            if not any(item["url"] == url for item in self.app_state.get("evidence_library", [])):
                self.app_state.setdefault("evidence_library", []).append({
                    "title": title, 
                    "url": url
                })
