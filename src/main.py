...
이 파일은 MVC 패턴으로 리팩토링된 크로스핏 코칭 데모 애플리케이션 메인 파일입니다.
"""
...
class CrossFitApp:
    ...
    def _bind_events(self, demo: gr.Blocks, components: Dict[str, Any]):
        ...
        # 주제별 자동 쿼리 이벤트
        components['terms_query_btn'].click(
            lambda: self.chatbot_controller.send_topic_query("용어/규칙/운동법"),
            None,
            [components['chat_view'], components['terms_status'], components['links_panel'], 
             components['evidence_full']]
        )
        
        components['diet_query_btn'].click(
            lambda: self.chatbot_controller.send_topic_query("식단/회복"),
            None,
            [components['chat_view'], components['diet_query_status'], components['links_panel'], 
             components['evidence_full']]
        )
        
        components['cert_query_btn'].click(
            lambda: self.chatbot_controller.send_topic_query("인증/챌린지 안내"),
            None,
            [components['chat_view'], components['cert_query_status'], components['links_panel'], 
             components['evidence_full']]
        )
        
        components['mentoring_query_btn'].click(
            lambda: self.chatbot_controller.send_topic_query("멘토링(초보 심리/동기)"),
            None,
            [components['chat_view'], components['mentoring_query_status'], components['links_panel'], 
             components['evidence_full']]
        )
